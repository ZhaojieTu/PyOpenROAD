...
_installOrTools() {
    ...
    local orToolsFound=false
    local orToolsPath=""
    
    # Use find command to locate installed OR-Tools
    set +e
    local foundPath=$(find /usr /opt /local -name "libortools.so.${orToolsVersionSmall}" 2>/dev/null | head -n 1)
    set -e
    
    # if [[ -n "$foundPath" ]]; then
    #     echo "OR-Tools is already installed"
    #     orToolsPath=$(dirname $(dirname "$foundPath"))
    ...
    echo "Installing OR-Tools from source..."
    ...
    ${cmakePrefix}/bin/cmake -S. -Bbuild \
        -DBUILD_DEPS:BOOL=ON \
        -DBUILD_EXAMPLES:BOOL=OFF \
        -DBUILD_SAMPLES:BOOL=OFF \
        -DBUILD_TESTING:BOOL=OFF \
        -DCMAKE_INSTALL_PREFIX=${orToolsPath} \
        -DCMAKE_CXX_FLAGS="-w" \
        -DCMAKE_C_FLAGS="-w"
...
_installTcl() {
    # Set variables
    tclVersion="8.6.13"
    tclPrefix=${PREFIX:-"/usr/local"}
    
    # Check if Tcl 8.6 is already installed
    if [[ -f ${tclPrefix}/bin/tclsh8.6 ]]; then
        echo "Tcl 8.6 is already installed."
        return 0
    fi
    
    echo "Installing Tcl ${tclVersion} from source..."
    cd "${baseDir}"
    # Download Tcl source code
    wget https://prdownloads.sourceforge.net/tcl/tcl${tclVersion}-src.tar.gz
    # Extract source code
    tar xzf tcl${tclVersion}-src.tar.gz
    cd tcl${tclVersion}/unix
    
    # Configure build
    ./configure --prefix=${tclPrefix} --enable-threads
    ...
    ln -sf ${tclPrefix}/bin/tclsh8.6 ${tclPrefix}/bin/tclsh
    ln -sf ${tclPrefix}/lib/libtcl8.6.so ${tclPrefix}/lib/libtcl.so
    
    # Update dynamic library configuration
    echo "${tclPrefix}/lib" > /etc/ld.so.conf.d/tcl.conf
    ldconfig
    
    cd "${lastDir}"
    echo "Tcl ${tclVersion} installed successfully."
    yum install -y tcl-tclreadline-devel tcl-tclreadline
}

_installRHELPackages() {
    arch=x86_64
    pandocVersion=2.19.2  # Use a more stable version

    # Configure Git to resolve TLS issues
    ...
    # Compile and install the latest version of Tcl
    lastDir="$(pwd)"
    _installTcl
    
    # Install Qt related dependencies
    yum -y install \
        qt-devel \
        qt5-qtbase-devel \
        qt5-qtimageformats

    # Install LLVM/Clang related dependencies
    yum -y install \
        llvm \
        llvm-devel \
        llvm-libs \
    ...
    if ! command -v pandoc &> /dev/null; then
        ...
        tar xvzf pandoc-${pandocVersion}-linux-amd64.tar.gz --strip-components 1 -C /usr/local/
        rm -rf pandoc-${pandocVersion}-linux-amd64.tar.gz
    fi
}
...
case "${os}" in
    "Ubuntu" )
        ...
        ;;
    "Red Hat Enterprise Linux" | "Rocky Linux" | "AlmaLinux" | *"CentOS"* )
